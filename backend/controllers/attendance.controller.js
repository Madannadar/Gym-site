...
export const deleteTodaysAttendanceByUserId = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0)
      return res.status(404).json({ message: "No attendance found for user today." });

    res.status(200).json({ message: "Today's attendance deleted", deleted: rows[0] });
  } catch (err) {
    console.error("❌ Error deleting today's attendance for user:", err.stack);
    res.status(500).json({ error: "Failed to delete today's attendance for user." });
  }
};
