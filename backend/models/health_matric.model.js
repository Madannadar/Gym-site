...
const recordHealthLog = async ({ user_id, log_type, value, log_date }) => {
  const query = `
...
const fetchRecentHealthLogs = async (user_id, log_type, limit = 5) => {
  const query = `
...
const updateHealthLogById = async (log_id, user_id, value, log_date) => {
  const query = `
...
const deleteHealthLogById = async (log_id, user_id) => {
  const query = `
...
const fetchHealthLogById = async (log_id, user_id) => {
  const query = `
    SELECT * FROM user_health_logs
    WHERE log_id = $1 AND user_id = $2;
  `;
