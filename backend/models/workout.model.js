...
// Exercise Functions
const recordExercise = async ({ name, description, muscle_group, units, created_by }) => {
  const query = `
    INSERT INTO exercises (name, description, muscle_group, units, created_by)
    VALUES ($1, $2, $3, $4, $5)
    RETURNING *;
  `;
  const values = [name, description, muscle_group, units, created_by];
...
const updateExerciseById = async (id, { name, description, muscle_group, units }) => {
  const query = `
    UPDATE exercises 
...
// Workout Functions
const recordWorkout = async ({ name, created_by, description, structure, score }) => {
  const query = `
    INSERT INTO workouts (name, created_by, description, structure, score)
    VALUES ($1, $2, $3, $4, $5)
    RETURNING *;
  `;
  const values = [name, created_by, description, JSON.stringify(structure), score];
  const { rows } = await db.query(query, values);
  return rows[0];
};
...
const fetchWorkoutById = async (id) => {
  ...
  const exerciseIds = workout.structure.map(item => item.exercise_id).filter(id => id);
...
const recordWorkoutLog = async ({ user_id, regiment_id, regiment_day_index, log_date, planned_workout_id, actual_workout, score }) => {
  const query = `
...
// Regiment Functions
const recordRegiment = async ({ created_by, name, description, workout_structure }) => {
  const query = `
    INSERT INTO regiments (created_by, name, description, workout_structure)
    VALUES ($1, $2, $3, $4)
    RETURNING *;
  `;
  const values = [created_by, name, description, JSON.stringify(workout_structure)];
  const { rows } = await db.query(query, values);
  return rows[0];
};
...
const fetchRegimentById = async (id) => {
  ...
  const workoutIds = regiment.workout_structure.map(day => day.workout_id).filter(id => id);
